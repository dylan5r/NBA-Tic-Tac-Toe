...
const normalizeId = (s: string) => s.toLowerCase().replace(/[^a-z0-9]+/g, "_");
...
export const buildPromptCatalog = (): NbaPrompt[] => {
  ...
  for (const star of teammateStars) {
    prompts.push({
      id: `teammate_${normalizeId(star)}`,
      type: "teammate",
      category: "teammate",
      difficulty: "medium",
      weight: weightByDifficulty("medium"),
      text: `Played with ${star}`,
      validation: { teammate: star }
    });
  }
